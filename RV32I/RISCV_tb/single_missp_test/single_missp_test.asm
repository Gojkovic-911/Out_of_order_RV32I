...
# -------------------- ONE BRANCH (misprediction) -------------------
# Condition: 100 < 50 → false, but static prediction = taken.
# So branch NOT taken – fall-through will execute, target is flushed.
blt     x2, x3, target      # x2=100, x3=50 → false

# ========== FALL‑THROUGH PATH (executed) ===========================
# Large block of ALU instructions that will be executed
addi    x2,  x0, 0xCA
addi    x3,  x0, 0xBA
xor     x4,  x2,  x3
and     x5,  x4,  x2
...
# ========== TARGET BLOCK (flushed – should NOT execute) ===========
target:
# This entire block is speculative and will be flushed on misprediction
addi    x1,  x0, 0xDF
addi    x2,  x0, 0xFE
add     x3,  x1,  x2
xor     x4,  x3,  x1
